...
protected:
	...
	public:
		...
		Elem *getNetx();		//Метод при вызове которого получаем указатель на следующий элемент (то на что указывает указатель next в стр12)
...
public:
	...
	Time *get(int);				//Метод для получения указателя на содержимое (на объект типа Time) используя индекс
